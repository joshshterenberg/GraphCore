...
def main():
    ...
    # constant EPS here, we're looking per layer
    EPS = 0.01 # determine the optimal value of this by running test_eps.py and getting the max val
    ...
    for layer in range(1,5):
        ...
        for i,(X,Y,sizeList) in enumerate(testDS):
            ...
            ####### isolate layers
            pred = torch.cat([pred[s] for s in range(len(pred)) if int(X[s,-1]) == layer]) ##who knows
            #######
    ...
    plt.plot(range(1, 5), LHC_arr, label='LHC')
    plt.plot(range(1, 5), perfect_arr, label='perfect')
    plt.xlabel("Layer")
    plt.ylabel("Percent")
    plt.title(f"Performance vs. Tracker Layer, EPS={EPS}")
    plt.legend()
    plt.savefig(f"layer_graph_{lsize}.png")
    plt.close()
