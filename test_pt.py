...
def main():
    ...
    testDS = OctopiDataset(glob.glob("/eos/user/n/nihaubri/OctopiNtuples/QCDMar7/OctopiNtuples_67.root"), featureBranches=featureBranches,labelBranches=labelBranches,batchsize=1)
    ...
    mva.to(device).eval()
    if WITH_GPU:
        model = torch.load(f"models/trained_gnn_{lsize}.pth")
    ...
    for EPS in EPS_arr:
        ...
        for i,(X,Y,sizeList) in enumerate(testDS):
            ...
            clusterizer = DBSCAN(eps=EPS, min_samples=1) # to match knn graph
